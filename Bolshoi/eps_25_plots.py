...
# %%
main = Eps_Parallel(inds, 0.10, 'gaussian', "iminuit")
...
for i in range(slices.shape[0] - 1):
    concentrations = temp[slices[i]: slices[i+1]]
    print(str(len(concentrations)) + '\n')
    med_c = np.median(concentrations)
    sigma = 1.4826 * mad(concentrations)
    temp2 = (concentrations - med_c) / sigma
    Y = np.append(Y, len(np.where(np.abs(temp2) > 3)[0]) / len(concentrations))
    print(Y)
