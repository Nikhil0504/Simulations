...
for i in range(19):
    condition_mass = data_points[(MASS_BINS[i+1] > data_points) & (data_points > MASS_BINS[i])]
    condition_age = hmass_scale[(MASS_BINS[i+1] > data_points) & (data_points > MASS_BINS[i])]
...
np.save('halofunc_points', data_points)
np.save('rvir_points', r_vir)
np.save('rsk_points', r_sk)
np.save('hmscale_points', hmass_scale)
np.save('mean_mass', mean_mass)
np.save('mean_age', mean_age)
np.save('std_mass', std_age)